...
public class LeetCode_127 {
    ...
    public int ladderLength(String beginWord, String endWord, List<String> wordList) {
        Queue<String> beginQueue = new LinkedList<>(); // 广度优先遍历当前层的队列
        beginQueue.add(beginWord);
        boolean[] marked = new boolean[wordList.size()];// 当前节点是否已经遍历过
        int layer = 1;
        while (!beginQueue.isEmpty()) { // 当前层存在
            layer++;
            int size = beginQueue.size(); // 当前层的大小
            while (size-- > 0) { // 遍历当前层
                String curWord = beginQueue.poll();
                for (int i = 0; i < wordList.size(); i++) { // 遍历所有节点
                    if (marked[i]) { // 已遍历节点
                        continue;
                    }
                    String dic = wordList.get(i);
                    if (canChange(dic, curWord)) { // 节点相连通
                        if (dic.equals(endWord)) { // 搜索找到结果
                            return layer;
                        }
                        beginQueue.add(dic); // 加到队尾
                        marked[i] = true; // 已访问
                    }
                }
            }
    ...
    // Double BFS
    public int ladderLength_DoubleBFS(String beginWord, String endWord, List<String> wordList) {
...
//        List<String> l = new ArrayList(beginQueue);
//        List<String> ll = new ArrayList(endQueue);
//        for (int j = 0; j < l.size(); j++) {
//            if (endQueue.contains(l.get(j))) {
//                return layer;
//            }
//            for (int jj = 0; jj < ll.size(); jj++) {
//                if (canChange(l.get(j), ll.get(jj))) {
//                    return layer + 1;
//                }
//            }
//        }
